...
int getsym() {
    ...
    if (ch >= 'a' && ch <= 'z') {
        ...
        do {
            ...
            // 四行意味着：
            // ch 的范围应该在
            // (a, b, c , ..., z) U (0, 1, ...., 9) 这个集合中 (也就是 a 到 z + 0 到 9)
        );
...
int gen(enum fct x, int y, int z) {
    if (cx >= cxmax) {
        printf("Program too loog");   /* 提示：程序过长 */
        return -1;
    }
    code[cx].f = x;
    code[cx].l = y;
 ...
 * (该部分的后跟符号),test 负责这项检测,而且负责当检测不通过时的补救措施
 * 程序在需要检测时指定当前需要的符号集合和补救用的集合(如之前未完成部分的后
 * 跟符号),以及检测不通过时的错误号
 *
 * s1: 我们需要的符号
...
int block(int lev,int tx,bool* fsys)
{
    ...
    table[tx0].size=dx;               /*声明部分中毎增加一条声明都会给dx增加1,声明
    ...
    /*语句后跟符号为分号或end*/
    memcpy(nxtlev,fsys,sizeof(bool)*symnum);/*每个后跟符号集都包含上层后跟符号集和,以便补救*/
    nxtlev[semicolon]=true;
    nxtlev[endsym]=true;
	...
	if (sym == ident)
	{
		...
		if (i == 0)
		{
            ...
            else
            {
                ...
                if (i != 0)      /*expression将执行一系列指令,但最终结果将会保存在栈顶,执行sto命令完成赋值*/
                {
            ...
            else
            {
                ...
                else
                {
                    if (sym == ifsym)		/*准备按照if语句处理*/
                    {
                        ...
                        code[cx1].a = cx;						/*经statement do处理后,cx为then后语句执行完的位置,它正是前面未指定的跳转地址*/
                    ...
                    else
                    {
                        if (sym == beginsym)      /*准备按照复合语句处理*/
                        {
                            ...
                            nxtlev[endsym] = true;       /*后跟符号为分号或end*/
                                                         /*循环调用语句处理函数直到下一个符号不是语句开始符号或收到end*/
    ...
    do{
        ...
        switch(i.f)
        {
            ...
            case opr:				/*数学、逻辑运算*/
                ...
                break;
            case lod:  /* 去相对当前过程的数据基地址为a的内存的值到栈顶 */
